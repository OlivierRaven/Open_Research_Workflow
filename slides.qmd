...
Never filter or subset data inside `index.qmd` — compute it in `analysis.qmd`, save it, then just read it back.
:::
...
```{.markdown}
<!-- index.qmd -->
mean = `r round(report_stats$mean_stone, 1)` mm, p = `r round(report_stats$pvalue, 3)`
```

. . .

Re-render → the manuscript updates itself. No stale numbers.
