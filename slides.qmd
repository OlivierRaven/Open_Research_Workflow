...
- Commit → **Publish to GitHub** (Public)
- `renv::init()` + `renv::snapshot()`
- zenodo.org → Settings → GitHub → toggle repo **ON**
:::
:::
...
::: {.column width="45%"}
::: {.step-list}
- zenodo.org → New Upload → fill metadata
- List every co-author with their **ORCID**
- Click **Reserve DOI** (do not publish yet)
- Add the DOI + GitHub link to the manuscript
:::
:::
...
::: {.column width="42%"}
::: {.step-list}
- Final commit: `"final version — accepted by [Journal]"`
- Sync to GitHub
- GitHub → Releases → **Create release** → tag `v1.0`
- zenodo.org → find draft → **Publish**
